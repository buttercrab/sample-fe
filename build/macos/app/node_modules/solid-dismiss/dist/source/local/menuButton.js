...
import { globalState, onKeyDownFocusIn, runLastAfterEvents, } from "../global/globalEvents";
...
export const onClickMenuButton = (state, e) => {
    const { timeouts, closeWhenMenuButtonIsClicked, focusedMenuBtn, onClickOutsideMenuButtonRef: onClickOutsideRef, setOpen, open, deadMenuButton, closeWhenClickingOutside, } = state;
...
export const onBlurMenuButton = (state, e) => {
    const { containerEl, focusedMenuBtn, overlay, setOpen, timeouts, menuBtnMouseDownFired, closeWhenDocumentBlurs, closeWhenClickingOutside, open, } = state;
    ...
    const run = () => {
        ...
        if ((!e.relatedTarget && activeElement && activeElement.tagName === "IFRAME",
            containerEl && containerEl.contains(activeElement)))
            return;
        if (!closeWhenDocumentBlurs && !document.hasFocus())
            return;
        if (globalState.closedBySetOpen)
            return;
        if (!currentMenuBtn.isConnected)
            return;
        if (isHiddenOrInvisbleShallow(currentMenuBtn)) {
            let clickIsOutside = false;
            state.menuBtnEls?.some((el) => {
                if (el === currentMenuBtn)
                    return false;
                if (isHiddenOrInvisbleShallow(el))
                    return false;
...
export const onFocusMenuButton = (state, e) => {
    const { closeWhenMenuButtonIsTabbed, timeouts, deadMenuButton, menuBtnEls, focusedMenuBtn, } = state;
    ...
    focusedMenuBtn.el = e.currentTarget;
...
export const addMenuButtonEventsAndAttributes = ({ state, menuButton, open, }) => {
...
export const setTargetAriaExpandTrue = (state) => {
...
export const setTargetAriaExpandFalse = (state) => {
    const { menuBtnEls, deadMenuButton } = state;
    if (!deadMenuButton)
        return;
    if (!menuBtnEls)
