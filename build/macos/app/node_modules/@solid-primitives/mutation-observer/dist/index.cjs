...
function createMutationObserver(initial, b, c) {
  ...
  const add = (el, options) => instance?.observe(el, utils.access(options) ?? defaultOptions);
  const start = () => {
    utils.asArray(utils.access(initial)).forEach((item) => {
...
exports.mutationObserver = (target, props) => {
