...
import { callHandler, getActiveElement, getScrollParent, getWindow, isIOS, isMac, mergeRefs, scrollIntoViewport, } from "@kobalte/utils";
import { Show, children, createEffect, createMemo, createSignal, on, onCleanup, splitProps, } from "solid-js";
...
export function DatePickerSegment(props) {
    let ref;
    ...
    const onBackspaceKeyDown = () => {
        if (numberParser().isValidPartialNumber(local.segment.text) &&
            !formControlContext.isReadOnly() &&
        ...
        switch (local.segment.type) {
            ...
            case "year": {
                ...
                if (local.segment.type === "hour" &&
                    ...
                    if (local.segment.value != null &&
                        local.segment.value >= 12 &&
                        numberValue > 1) {
                        numberValue += 12;
                    }
                }
                else if (local.segment.maxValue != null &&
    ...
    createEffect(() => {
        ...
        if (resolvedDateValue) {
            if (local.segment.type === "month" && !local.segment.isPlaceholder) {
                ...
                setTextValue((prev) => monthTextValue !== prev
                    ? `${prev} – ${monthTextValue}`
                    : monthTextValue);
            }
            else if (local.segment.type === "hour" &&
                !local.segment.isPlaceholder) {
    ...
    return (<Show when={local.segment.type !== "literal"} fallback={<Polymorphic as="div" aria-hidden={true} data-separator="" {...others}>
					{local.segment.text}
				</Polymorphic>}>
			<SpinButton.Root ref={mergeRefs((el) => (ref = el), local.ref)} tabIndex={formControlContext.isDisabled() ? undefined : 0} value={local.segment.value} textValue={textValue()} minValue={local.segment.minValue} maxValue={local.segment.maxValue} validationState={formControlContext.validationState()} required={formControlContext.isRequired()} disabled={formControlContext.isDisabled()} readOnly={formControlContext.isReadOnly() || !local.segment.isEditable} contentEditable={isEditable()} inputMode={inputMode()} 
    // @ts-ignore
    autocorrect={isEditable() ? "off" : undefined} autoCapitalize={isEditable() ? "off" : undefined} spellcheck={isEditable() ? false : undefined} enterkeyhint={isEditable() ? "next" : undefined} style={{ "caret-color": "transparent" }} aria-label={ariaLabel()} aria-labelledby={inputContext.ariaLabelledBy()} aria-describedby={ariaDescribedBy()} data-placeholder={local.segment.isPlaceholder ? "" : undefined} onKeyDown={onKeyDown} onBeforeInput={onBeforeInput} onInput={onInput} onFocus={onFocus} onIncrement={onIncrement} onDecrement={onDecrement} onIncrementPage={onIncrementPage} onDecrementPage={onDecrementPage} onDecrementToMin={onDecrementToMin} onIncrementToMax={onIncrementToMax} {...datePickerContext.dataset()} {...formControlContext.dataset()} {...others} {...touchPropOverrides()}>
