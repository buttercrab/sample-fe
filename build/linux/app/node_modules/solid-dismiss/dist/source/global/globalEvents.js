...
import { getFirstVisibleMountedPopupFromSafeList, removeEventsOnActiveMountedPopup, } from "../local/thirdPartyPopup";
...
export const onDocumentPointerDown = (e) => {
...
export const onWindowBlur = (e) => {
    ...
    setTimeout(() => {
        ...
        if (!document.hasFocus()) {
            if (difference < 50) {
                checkThenClose(dismissStack, (item) => ({ item, continue: true }), (item) => {
                    const { setOpen } = item;
    ...
    setTimeout(() => {
        ...
        if (!activeElement || activeElement.tagName !== "IFRAME") {
            checkThenClose(dismissStack, (item) => ({ item, continue: true }), (item) => onBlurWindow(item));
            return;
        }
...
export const onKeyDownFocusIn = (e) => {
    if (e.key !== "Tab")
        return;
    setTimeout(() => {
        const activeElement = document.activeElement;
...
export const onKeyDown = (e) => {
    const { setOpen, menuBtnEls, cursorKeys, closeWhenEscapeKeyIsPressed, focusElementOnClose, ignoreMenuPopupWhenTabbing, focusSentinelAfterEl, focusSentinelBeforeEl, mountedPopupsSafeList, } = dismissStack[dismissStack.length - 1];
    if (e.key === "Tab") {
    ...
    if (mountedPopupsSafeList && mountedPopupsSafeList.length) {
        ...
        if (el) {
            window.setTimeout(() => {
                if (!el.isConnected || isHiddenOrInvisbleShallow(el))
                    return;
                close();
            }, 100);
            return;
